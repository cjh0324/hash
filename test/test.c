...
int test_cache(size_t attempts, size_t lower_cache_size, int * latencies, size_t max_latency) {
    ...
    size_t i;
    for (i = 0; i < lower_cache_size; i += sysconf(_SC_LEVEL1_DCACHE_LINESIZE)) {
        random_data[i] = 1;
    }
...
int main() {
    ...
    for (i = 0; i < num_data_caches; i++) {
        ...
        fprintf(stderr, "Cache Level %i has latency %d cycles\n", i+1, maxIdx - empty_cycles);
        // Printf latencies
        // printf("This is Cache ID %i latencies\n", i);
        // for(j = 0; j < sizeof(latencies) / sizeof(*latencies); j++)
        //     printf("%d ", latencies[j]);
        // printf("\n");
    ...
    // Printf latencies
    // for(i = 0; i < sizeof(latencies) / sizeof(*latencies); i++)
    //     printf("%d ", latencies[i]);
    // printf("\n");

    // Calculate sum of latencies(must equal attempts)
    // int xsum = 0;
    // for(i = 0; i < sizeof(latencies) / sizeof(*latencies); i++)
    //       xsum = xsum + latencies[i]; // same as sum += arr[i];
    // printf("is latencies ok? %d\n", xsum);
